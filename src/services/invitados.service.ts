...
export const findByPhone =  async (req: Request, res: Response, next: Function) => {
  try {
...
const createSearchPatterns = (name: string): string[] => {
  ...
  const uniquePatterns = [...new Set(patterns)].filter(
...
const searchByNonConsecutiveWords = async (
  searchWords: string[]
): Promise<any> => {
  ...
  return await InvitadoModel.findOne({name: fullRegex,});
};


//TODO: ACOMODAR ESTA MIERDA
/*
// Buscar invitado por nombre y teléfono
export const findByPhone = async (name: string, phone: string): Promise<Invitado | null> => {
  const searchPatterns = createSearchPatterns(name);
  let invitado = null;
  ...
  if (!invitado) {
    console.log(
      "🔍 Búsqueda por nombre falló, intentando solo por teléfono..."
    );
  ...
  if (!invitado) {
    console.log(
      "🔍 Búsqueda por teléfono falló, intentando solo por nombre..."
    );
    ...
    for (const pattern of searchPatterns) {
      ...
      if (pattern.length > 3) {
        ...
        if (invitado) {
          console.log(
            "✅ Invitado encontrado solo por nombre con patrón:",
            invitado.name
          );
          break;
        }
  ...
  if (!invitado) {
    console.log(
      "🔍 Búsqueda por patrones falló, intentando por palabras no consecutivas..."
    );

    const searchWords = normalizeName(name)
      .split(" ")
    ...
    if (searchWords.length > 1) {
      ...
      if (invitado) {
        console.log(
          "✅ Invitado encontrado por palabras no consecutivas:",
          invitado.name
        );
      }
    }
